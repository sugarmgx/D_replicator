...
- **入れ子(Replicator in Replicator)** — 非破壊
...
`build_extension.ps1` が `src/replicator.py` を `extension/__init__.py` にコピーし(**単一ファイル拡張**)、
Blender 公式ツールで `D_Replicator-<ver>.zip` を作ります
(`extension/__init__.py` は生成物のため Git では追跡しません)。
...
extension/             … 配布パッケージ(__init__.py=本体[生成] + manifest + LICENSE)
